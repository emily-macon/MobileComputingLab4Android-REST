...
public class BackgroundSubscribeIntentService extends IntentService {
    ...
    private static final String BASE_URL = "http://66.253.134.126:8080";
    ...
    AsyncHttpResponseHandler asyncHandler = new AsyncHttpResponseHandler() {
        ...
        public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
            Log.i("REST","200: Success");
        }

        @Override
        public void onFailure(int statusCode, Header[] headers, byte[] responseBody, Throwable error) {
            Log.i("REST","404: Failure");
        }
    };
    @Override
        ...
        if (intent != null) {
            Nearby.Messages.handleIntent(intent, new MessageListener() {
                ...
                public void onFound(Message message) {
                    Log.i("Background", "Inc: "+message.getType());
                    Utils.saveFoundMessage(getApplicationContext(), message);
                    updateNotification();
                    // INSERT REST REQUEST HERE
                    if(message.getNamespace().equals("swift-capsule-162020")) {
                        client.post(BASE_URL + "/inc/" + message.getType(), asyncHandler);
                    }
                }

                @Override
                public void onLost(Message message) {
                    Log.i("Background", "Dec: "+message.getType());
                    Utils.removeLostMessage(getApplicationContext(), message);
                    updateNotification();
                    // INSERT REST REQUEST HERE
                    if(message.getNamespace().equals("swift-capsule-162020")) {
                        client.post(BASE_URL + "/dec/" + message.getType(), asyncHandler);
                    }
                }
            });
        }
